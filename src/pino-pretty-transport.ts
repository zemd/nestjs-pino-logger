...
function formatStack(title: string, stack: string): string {
  const formattedStack = cleanStack(stack, {pretty: true, basePath: process.cwd()});
  const realTitle = title ? `Error: ${title}` : '';
  return `${colorsMap["red"]?.start}${colorsMap["bold"]?.start}${realTitle
  ?? ''}${colorsMap["bold"]?.end} ${colorsMap["dim"]?.start}${formattedStack.slice(
    realTitle?.length)}${colorsMap["dim"]?.end}${colorsMap["red"]?.end}`;
}
...
export default (opts: any) => {
  ...
  return pinoPretty({
    ...
    messageFormat: (log: LogDescriptor): string => {
      try {
        const context = log["context"] as string ?? 'unknown';
        const time = log["time"] as number;
        const msg = log["msg"] as string;
        const formattedMsg = log["formattedMsg"] as string ?? msg;
        const logLevel = log["level"] as number;
        const levelLabel = levelsMap[logLevel];
        const err = log["err"] as { type: 'Error', stack: string, message: string };
        ...
        if (log["req"] && log["res"]) {
          ...
          const formattedResponseTime = `(⮂ ${colorsMap["green"]?.start}${ms(
            responseTime)}${colorsMap["green"]?.end})`;
          ...
          output.push(formatMsg(cleanStack(formattedMsg, {pretty: false, basePath: process.cwd()}), prefix));
        }

        if (err) {
          output.push(
            `${formatStack(err.message, err.stack)}${msg ? `\n${prefix} ${formatMsg(msg, prefix)}` : ''}`);
        }
